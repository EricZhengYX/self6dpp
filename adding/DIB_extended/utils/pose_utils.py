...
def compose_RT_torch(R: torch.Tensor, T: torch.Tensor):
    ...
    R : torch tensor, shape (b, 3, 3)
        Rotation matrix
    T : torch tensor, shape (b, 3, 1)
        Translations

    Returns
    -------
    H : torch tensor, shape (b, 4, 4)
        Affine transformation matrix

    '''
    assert isinstance(R, torch.Tensor) and isinstance(T, torch.Tensor), 'R&T should be both in type of torch.Tensor'
    assert len(R.shape) == 3 and R.shape[1:] == (3, 3), 'R should have a shape of b*3*3'
    assert len(T.shape) == 3 and T.shape[1:] == (3, 1), 'T should have a shape of b*3*1'
    b, _, _ = R.shape;
    b1, _, _ = T.shape
    assert b == b1, 'Batch sizes of R and T are not the same'
    device = R.device;
    device1 = T.device
    assert device == device1, 'Devices of R and T are not the same'

    A = torch.dstack((R, T))
    padding = torch.tensor([0, 0, 0, 1]).to(device)[None, None, :].expand(b, -1, -1)
