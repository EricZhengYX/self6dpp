...
class GDRN_DoubleMask_DoubleVF(nn.Module):
    def __init__(self, cfg, backbone, geo_head_net, neck=None, pnp_net=None):
        super().__init__()
        assert (
            cfg.MODEL.POSE_NET.NAME == "GDRN_double_mask_double_vf"
        ...
        (
            ...
            self.mask_out_dim,
            self.region_out_dim,
        ) = get_xyz_doublemask_region_out_dim(cfg)
    ...
    def train(self, mode=True):
        ...
        super(GDRN_DoubleMask_DoubleVF, self).train(mode)
        cfg = self.cfg
        if cfg.MODEL.FREEZE_BN:
            logger.info("GDRN_DoubleMask_DoubleVF turns into training mode, but freeze bn...")
    ...
    def forward(
        ...
        if pnp_net_cfg.TRANS_TYPE == "centroid_z":
            ...
            pred_ego_rot, pred_trans = pose_from_pred(
                ...
                eps=1e-4,
                is_allo="allo" in rot_type,
                is_train=do_loss or do_self,  # Only "do_loss" in GDRN_DoubleMask
            )
        else:
...
def build_model_optimizer(cfg, is_test=False):
    ...
    # build model
    model = GDRN_DoubleMask_DoubleVF(
        cfg, backbone, neck=neck, geo_head_net=geo_head, pnp_net=pnp_net
    )
    if net_cfg.USE_MTL:
        params_lr_list.append(
            {
