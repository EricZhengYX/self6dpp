...
def transform_instance_annotations(
    ...
    *,
    ...
    bbox = BoxMode.convert(
        annotation[bbox_key], annotation["bbox_mode"], BoxMode.XYXY_ABS
    )
    ...
    def read_data_train(self, dataset_dict):
        ...
        if self.with_depth:
            ...
            if self.aug_depth:  # NOTE: augment depth
                depth[depth == 0] = np.random.normal(
                    np.median(depth[depth == 0]), 0.1, depth[depth == 0].shape
                )
            ...
            dataset_dict["depth"] = torch.as_tensor(
                depth.reshape(im_H, im_W).astype("float32")
            )
...
def load_detections_with_poses_into_dataset(
    ...
    for i, record_ori in enumerate(dataset_dicts):
        ...
        for det in dets_i:
            ...
            if with_pose:
                log_first_n(
                    logging.INFO, "load detections with pose_est and pose_refine", n=1
                )
        ...
        for obj, cur_annos in obj_annotations.items():
            ...
            sel_annos = [
                ann
                for _, ann in sorted(
                    zip(scores, cur_annos), key=lambda pair: pair[0], reverse=True
                )
            ][:top_k_per_obj]
...
def build_gdrn_self_train_loader(cfg, dataset_names, train_objs=None):
    ...
    if cfg.MODEL.SELF_TRAIN and cfg.MODEL.LOAD_DETS_TRAIN:
        ...
        for dataset_name, dataset_dicts, det_file in zip(
            dataset_names, dataset_dicts_list, det_files
        ):
    ...
    dataset = GDRN_Self_DatasetFromList(
        cfg, split="train", lst=final_dataset_dicts, copy=False, flatten=True
    )
