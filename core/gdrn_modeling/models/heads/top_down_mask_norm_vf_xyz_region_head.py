...
class TopDownMaskNormVFXyzRegionHead(nn.Module):
    def __init__(
        self,
        in_dim,
        ...
        vf_out_dim=2 * 2 * 16,  # {u, v} * {vis, full} * N_fps
        norm_out_dim=3*2,  # 3*{vis, full}
    ):
        """
        Args:
        ...
        self.vf_num_classes = vf_num_classes
        self.norm_num_classes = norm_num_classes

        self.mask_out_dim = mask_out_dim
        self.xyz_out_dim = xyz_out_dim
        self.region_out_dim = region_out_dim
        self.vf_out_dim = vf_out_dim
        self.norm_out_dim = norm_out_dim
        ...
        if self.out_layer_shared:
            out_dim = (
                ...
                + self.vf_out_dim * self.vf_num_classes
                + self.norm_out_dim * self.norm_num_classes
            )
            ...
            self.vis_norm_out_layer = nn.Conv2d(
                feat_dim,
                (self.norm_out_dim // 2) * self.norm_num_classes,
                kernel_size=out_kernel_size,
                padding=(out_kernel_size - 1) // 2,
                bias=True,
            )
            self.full_norm_out_layer = nn.Conv2d(
                feat_dim,
                (self.norm_out_dim // 2) * self.norm_num_classes,
                kernel_size=out_kernel_size,
                padding=(out_kernel_size - 1) // 2,
            ...
            self.vis_vf_out_layer = nn.Conv2d(
                feat_dim,
                (self.vf_out_dim // 2) * self.vf_num_classes,
                kernel_size=out_kernel_size,
                padding=(out_kernel_size - 1) // 2,
                bias=True,
            )
            self.full_vf_out_layer = nn.Conv2d(
                feat_dim,
                (self.vf_out_dim // 2) * self.vf_num_classes,
                kernel_size=out_kernel_size,
                padding=(out_kernel_size - 1) // 2,
        ...
        if self.out_layer_shared:
            ...
            normal_init(self.vis_mask_out_layer, std=0.01)
            normal_init(self.full_mask_out_layer, std=0.01)
            normal_init(self.xyz_out_layer, std=0.01)
            normal_init(self.region_out_layer, std=0.01)
            normal_init(self.vis_vf_out_layer, std=0.01)
            normal_init(self.full_vf_out_layer, std=0.01)
    ...
    def forward(self, x):
        ...
        if self.out_layer_shared:
            ...
            region_dim = self.region_out_dim * self.region_num_classes
            vf_dim = self.vf_out_dim * self.vf_num_classes
            norm_dim = self.norm_out_dim * self.norm_num_classes

            vis_mask = out[:, : (mask_dim // 2), :, :]
            full_mask = out[:, (mask_dim // 2) : mask_dim, :, :]

            sum_dim = mask_dim
            xyz = out[:, sum_dim : sum_dim + xyz_dim, :, :]
            ...
            sum_dim += vf_dim // 2
            raw_full_vf = out[:, sum_dim : sum_dim + (vf_dim // 2), :, :]
            full_vf = _post_process_vf(raw_full_vf)

            sum_dim += vf_dim // 2
            raw_vis_norm = out[:, sum_dim : sum_dim + (norm_dim // 2), :, :]
            vis_norm = _post_process_norm(raw_vis_norm)

            sum_dim += norm_dim // 2
            raw_full_norm = out[:, sum_dim:, :, :]
            full_norm = _post_process_norm(raw_full_norm)
            ...
            vis_norm = _post_process_norm(self.vis_norm_out_layer(x))
            full_norm = _post_process_norm(self.full_norm_out_layer(x))
        ...
        return vis_mask, full_mask, vis_vf, full_vf, vis_norm, full_norm, coor_x, coor_y, coor_z, region
...
def _post_process_norm(raw_norm: torch.Tensor):
    """

    @param raw_norm: B * 3 * W * H
    @return: same sized
    """
