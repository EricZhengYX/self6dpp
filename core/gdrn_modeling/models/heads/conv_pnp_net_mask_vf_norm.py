...
class ConvPnPNetMaskNormVF(nn.Module):
    def __init__(
        self,
        nIn,
    ...
    def forward(self, coor_feat, region, mask, vf, norm, extents=None):
        '''

        @param coor_feat: 2D-3D corr + coord, B*5*W*H
        @param region: regions w/o bg, 64(for example)*W*H
        @param mask: full + vis, 2*W*H
        @param vf: full+vis, 64(when using 16 fps)*W*H
        @param norm: full+vis, (2*3)*W*H
        @param extents: B*3
        @return: R, B*(6 or 4); t, B*3
        '''
        ...
        # vf
        x = torch.cat([x, vf.view(bs, -1, fh, fw)], dim=1)
